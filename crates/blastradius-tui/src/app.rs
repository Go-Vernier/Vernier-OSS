...
use blastradius::history::{self, History};
use blastradius::{Analysis, Blast, Change, Edge, Service, ServiceRole, blast, git};
...
impl App {
    ...
    pub fn new(analysis: Analysis, options: Options) -> Self {
        ...
        let overview = blastradius::format_repo_report(&analysis, false)
            .lines()
            .map(str::to_string)
            .collect();
