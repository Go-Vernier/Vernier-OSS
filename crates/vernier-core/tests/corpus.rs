...
fn check_repo(name: &str, repo: &Path, expected: &Expected) -> Vec<String> {
    ...
    let analysis = vernier::analyze(repo).unwrap();
    ...
    let by = |c: vernier::Confidence| json.edges.iter().filter(|e| e.confidence == c).count();
    eprintln!(
        "{name:<32} {:>4} code {:>4} infra {:>4} edges ({}s/{}i/{}u) {:>4} unresolved {:>7.1} ms  {}",
        code.len(),
        infra.len(),
        json.edges.len(),
        by(vernier::Confidence::Static),
        by(vernier::Confidence::Inferred),
        by(vernier::Confidence::Uncertain),
