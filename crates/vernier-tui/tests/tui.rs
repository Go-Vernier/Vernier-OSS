...
use vernier::{Analysis, ChangeKind, blast};
use vernier_tui::app::{Pane, Tab};
use vernier_tui::{Action, App, Options};
...
fn render(app: &App, width: u16, height: u16) -> String {
    ...
    terminal
        .draw(|frame| vernier_tui::ui::draw(frame, app))
        .unwrap();
...
fn the_walk_tree_hangs_every_reached_service_off_its_path() {
    ...
    app.update(key(KeyCode::Enter));
    let b = app.blast.as_ref().unwrap();
    let rows = vernier_tui::tree::rows(b);
    ...
    assert_eq!(rows[0].kind, vernier_tui::tree::Kind::Root);
    for (i, r) in b.reached.iter().enumerate() {
        let row = rows
            .iter()
            .find(|row| row.kind == vernier_tui::tree::Kind::Reached(i))
    ...
    assert!(matches!(
        ...
        vernier_tui::tree::Kind::Reached(_)
    ));
}
...
fn infrastructure_says_the_walk_starts_from_code() {
    ...
    assert_eq!(service.role, vernier::ServiceRole::Infrastructure);
    app.update(key(KeyCode::Enter));
    assert_eq!(app.tab, Tab::Services);
    assert_eq!(
        app.status.as_deref(),
...
fn a_change_from_the_command_line_opens_on_the_blast_tab() {
    ...
    let change = vernier::git::pull_request(&repo.root, 7).unwrap();
    let app = App::new(
        analysis(&repo.root),
        Options {
            depth: 2,
