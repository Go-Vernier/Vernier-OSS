...
use vernier::{Blast, ChangeKind, Confidence, Hop, blast::NOT_REACHED};
...
fn hop_tail(hop: &Hop) -> String {
    let words = vernier::hop_words(hop);
    words
        ...
        Kind::Reached(i) => {
            ...
            lines.extend(r.path.iter().enumerate().map(|(n, hop)| {
                Line::from(vec![
                    ...
                    Span::styled(vernier::hop_words(hop), t.confidence(hop.confidence)),
                ])
            }));
            lines.push(Line::styled(
                format!(
